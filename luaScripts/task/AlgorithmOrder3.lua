...
function checkRunningCount(orders)
    local runningCount = 0
    local sz = orders:size()
    for i = orders:size() - 1, 0, -1 do -- 考查所有没有到终止状态的委托
        local order = orders:at(i)
        if not ((isGreater(order.m_dCompleteTime, 0) and not isInvalidDouble(order.m_dCompleteTime)) or (isGreater(order.m_dCancelTime, 0) and not isInvalidDouble(order.m_dCancelTime))) then
...
function run(obj)
    ...
    while true do
        ...
        if wait then
            ...
            local runningCount = checkRunningCount(orders)
            ...
            local now = os.time()
            if now - lastOrderTime >= param.m_dPlaceOrderInterval or runningCount == 0 then
