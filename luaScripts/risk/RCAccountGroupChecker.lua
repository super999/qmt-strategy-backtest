...
function init()
    ...
    m_strLogTagOriginal = "[ accountgroup_check : group=" .. tostring(m_nGroupId) ..  " ] "
...
function check(param)
    ...
    if m_nId == XT_COrderCommand or m_nId == XT_CCreateTaskReq then
        res = rcCheckAccountGroupReverse(param.m_stockParams, m_nGroupId)
        if not res:isSuccess() then
            return res
        end
    elseif m_nId == XT_COrderInfo then
        res = rcCheckAccountGroupBucket(param, m_nGroupId)
        if not res:isSuccess() then
            return res
        end
    end
