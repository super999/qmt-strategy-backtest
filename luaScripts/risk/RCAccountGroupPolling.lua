...
function init()
    ...
    m_strLogTag = "[ accountgroup_polling : group=" .. tostring(m_nGroupId) ..  " ] "
...
local function pollingGroupAssets()
    local msgHeader = '[账号组风控] ' .. m_accountGroupTag
    ...
    for iRule = 1, size, 1 do
        ...
        if interval > 0 then
            ...
            if 0 == m_pollingCnt % interval or (thisPolling - lastPolling >= 1) then    
                ...
                local res, msg = pollingSubFundRule(XT_RC_SCOPE_TYPE_ACCOUNTGROUP, CCodeOrderParamList().m_params, false, rc, msgHeader, tostring(m_nGroupId))
            else
